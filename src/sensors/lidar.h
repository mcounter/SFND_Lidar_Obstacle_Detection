...
struct Ray
{
	...
	void rayCast(const std::vector<Car>& cars, double minDistance, double maxDistance, typename pcl::PointCloud<PointT>::Ptr& cloud, double slopeAngle, double sderr)
	{
		// reset ray
		castPosition = origin;
		castDistance = 0;
		...
		if((castDistance >= minDistance)&&(castDistance<=maxDistance))
		{
			...
			PointT point;
			point.x = castPosition.x+rx*sderr;
			point.y = castPosition.y+ry*sderr;
			point.z = castPosition.z+rz*sderr;
...
struct Lidar
{
	...
	Lidar(std::vector<Car> setCars, double setGroundSlope)
		: cloud(new pcl::PointCloud<PointT>), position(0,0,2.6)
	{
		...
		for(double angleVertical = steepestAngle; angleVertical < steepestAngle+angleRange; angleVertical+=angleIncrement)
		{
			...
			{
				Ray<PointT> ray(position,angle,angleVertical,resoultion);
				rays.push_back(ray);
			}
		}
	}
	...
	typename pcl::PointCloud<PointT>::Ptr scan()
	{
		cloud->points.clear();
		...
		cout << "Ray casting took " << elapsedTime.count() << " mcs" << endl;
